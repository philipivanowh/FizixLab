...
export default class Scene {
  constructor(iterations = MIN_ITERATIONS) {
    this.objects = [];
  ...
  addBody(body) {
    if (!(body instanceof PhysicsBody)) {
      throw new Error("Only PhysicsBody instances can be added to the scene.");
    }
  ...
  step(time, iterations = this.iterations) {
    const clampedIterations = clamp(
  ...
  resolveCollision(bodyA, bodyB, normal) {
    const relativeVelocity = bodyB
      .getLinearVelocity()
      .subtract(bodyA.getLinearVelocity());
    ...
    if (bodyA.invMass > 0) {
      bodyA.setLinearVelocity(
        bodyA.getLinearVelocity().subtract(impulse.multiply(bodyA.invMass)),
      );
    }

    if (bodyB.invMass > 0) {
      bodyB.setLinearVelocity(
        bodyB.getLinearVelocity().add(impulse.multiply(bodyB.invMass)),
      );
    }
  }
